...
int calculate_smid(const string &seqA, const string &seqB,
                   vector<int> &prev1, vector<int> &prev2, vector<int> curr, 
                   int len_diag, int d, int state){
    ...
    __m256i curr_vec = _mm256_stezero_si256();
    __256i prev1_vec = _mm256_loadu_si256((__256*)&prev1[d]);
    __256i prev2_vec = _mm256_loadu_si256((__256*)&prev2[d]);
...
void SmithWaterman_parallel(const string &seqA, const string &seqB, int &max_score, int &max_i, int &max_j) {
    ...
    {        
        ...
        for (int diag = 2; diag < rows + cols - 1; ++diag)
        {
            ...
            {
                calculate_smid(seqA, seqB, prev1, prev2, curr, len_diag, d, state);
                // int i = start_row + d;
                // int j = diag - i;
                // if(i == 0 || j == 0) curr[d] = 0;

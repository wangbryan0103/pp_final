...
int calculate_parallel(const string &seqA, const string &seqB, vector<vector<int>> &diagonals, int diag, int d, int i, int j, int state){
    ...
    {
        int index = (state == BEFORE_MAX)? (d-1) : (state == NOW_MAX)? d : (d+1);
        curr = diagonals[diag-2][index];  //prev1[index] + match_score;
    }else
    {
        ...
        if(state == BEFORE_MAX){
            curr = max(diagonals[diag-1][d-1] + gap_score, max(diagonals[diag-1][ d ] + gap_score, max(diagonals[diag-1][d-1] + mismatch_score, 0)));
        }else if (state == NOW_MAX){
            curr = max(diagonals[diag-1][ d ] + gap_score, max(diagonals[diag-1][d-1] + gap_score, max(diagonals[diag-1][ d ] + mismatch_score, 0)));
        }else{
            curr = max(diagonals[diag-1][ d ] + gap_score, max(diagonals[diag-1][d+1] + gap_score, max(diagonals[diag-1][d+1] + mismatch_score, 0)));
        }
    }
    return curr;
}
...
void SmithWaterman_parallel(const string &seqA, const string &seqB, int &max_score, int &max_i, int &max_j) {
    ...
    vector<vector<int> > diagonals(rows+cols, vector<int>(rows, 0));
    ...
    {        
        ...
        for (int diag = 2; diag < rows + cols - 1; ++diag){
            ...
            cout << "emd_row:" << end_row << endl << endl;

            int state = (start_row == 0)? BEFORE_MAX : (start_row == 1)? NOW_MAX : ATFER_MAX;
            ...
            for(int d = 0; d < len_diag; ++d)
            {
                int i = start_row + d;
                int j = diag - i;
                if(i == 0 || j == 0) diagonals[diag][d] = 0;//curr[d] = 0;
                else diagonals[diag][d] = calculate_parallel(seqA, seqB, diagonals, diag, d, i, j, state);

                cout << " | i=" << i << " | j=" << j  << " | curr["<<d<<"]=" << diagonals[diag][d] << endl;

                if(diagonals[diag][d] > local_max_score)
                {
                    local_max_score = diagonals[diag][d];
                    local_i = i;
                    local_j = j;
                }
            }
